...
public class ParseCommandHelper
{
    ...
    public Graph parseAndExecuteCommand(ParserInputHelper existGraphBuilder, String headline) throws Exception {
        ...
        if(cmdhead[0].equals(possibleinputhead[0]))
        {
            ...
            if(cmdhead[1].equals(possiblecm[0]))//vertex --all label type
            {
                ...
                if(!existGraphBuilder.judgeExist(this.label))
                    ans = cmdIOConstructorVertex_Adder(existGraphBuilder);
                else
    ...
    private Graph cmdIOConstructorVertex_Adder(ParserInputHelper pih) throws Exception {
        Scanner sc = new Scanner(System.in);
